<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-900">Purchases</h2>
            <a href="{{ route('purchases.create') }}" class="bg-gray-900 hover:bg-black text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
                + Add Purchase
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                    <table id="purchases-table" data-dt class="admin-table">
                        <thead>
                            <tr>
                                <th>Product</th>
                                <th>Category</th>
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="w-11 h-11 rounded-lg bg-gray-100 overflow-hidden shrink-0 border border-gray-200">
                                                @if($purchase->product->image)
                                                    <img src="{{ Storage::url($purchase->product->image) }}" class="w-full h-full object-cover">
                                                @else
                                            ...
                                            <div>
                                                <div class="font-semibold text-gray-900">
                                                    ...
                                                    @if($purchase->product->is_featured)<span class="text-yellow-400 ml-1">★</span>@endif
                                                </div>
                                                @if($product->name_kh)
